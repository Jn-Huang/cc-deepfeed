...
status: ## Show dashboard of all topics and feeds
	uv run python3 feed.py status

prune: ## Prune old entries (usage: make prune KEEP=50)
	uv run python3 feed.py prune --keep $(or $(KEEP),50)

publish: ## Publish feeds to gh-pages and ping WebSub hub
	@BASE_URL=$$(uv run python3 -c "import yaml; print(yaml.safe_load(open('config.yaml')).get('settings',{}).get('base_url',''))"); \
